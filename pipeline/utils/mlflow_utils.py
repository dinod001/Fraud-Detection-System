...
class MLflowTracker:
    ...
    def setup_mlflow(self):
        ...
        experiment_name = self.config.get('experiment_name', 'fraud_detection_system')
    ...
    def log_training_metrics(self, model, training_metrics: Dict[str, Any], model_params: Dict[str, Any]):
        ...
        try:
            ...
            # Filter for numeric metrics only (MLflow doesn't support strings or arrays in log_metrics)
            numeric_metrics = {
            ...
            mlflow.sklearn.log_model(
                ...
                registered_model_name=self.config.get('model_registry_name', 'fraud_detection_system')
            )
    ...
    def log_inference_metrics(self, predictions: np.ndarray, probabilities: Optional[np.ndarray] = None, 
        ...
        try:
            ...
            inference_metrics = {
                ...
                'prediction_distribution_non_fraud': int(len(predictions) - np.sum(predictions))
            }
    ...
    def load_model_from_registry(self, model_name: Optional[str] = None, 
        ...
        try:
            if model_name is None:
                model_name = self.config.get('model_registry_name', 'fraud_detection_model')
